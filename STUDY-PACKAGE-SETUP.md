...
The setup script below is used to install the **SkeletonCohortCharacterization** package. You will need to modify this setup script as follows:
                                                                                              
- Set the `projectRootFolder` variable to the directory specific to your environment. In this example we are using `C:/SkeletonCohortCharacterization`. This root folder will serve a few purposes:
...
projectFolder <- "C:/SkeletonCohortCharacterization"
...
download.file("https://raw.githubusercontent.com/ohdsi-studies/SkeletonCohortCharacterization/master/renv.lock", "renv.lock")
...
# finally either:
# A) install the SkeletonCohortCharacterization package
install.packages('devtools')
devtools::install_github('ohdsi-studies/SkeletonCohortCharacterization')

# or 
#B) build the package using rtools (if you have a local copy)
...
You can safely continue by pressing 'y' after this prompt since the renv.lock file is downloaded from the **SkeletonCohortCharacterization** GitHub code repository. Once the installation is complete, you may need to restart R (if you are working outside of RStudio) and you should see this message:

````
Project 'C:/SkeletonCohortCharacterization' loaded. [renv 0.x.y]
````
